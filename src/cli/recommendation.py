...
def cmd_recommendation_compute(
    ...
    run = _start_pipeline_run(session, "recommendation_compute")
    if tickers:
    ...
    for company in companies:
        outcome = compute_recommendation(session, company.ticker)
        session.commit()
    ...
    print(f"\nRecommendation-compute summary: {len(companies)} companies, {total_written} written, {total_skipped} skipped")
    return 0

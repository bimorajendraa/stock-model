"""CLI command implementations for fundamentals ingestion (spec section 3.3)."""
from __future__ import annotations

from sqlalchemy import select
from sqlalchemy.orm import Session

from src.cli.market import _finish_pipeline_run, _start_pipeline_run
from src.config.settings import Settings
from src.data_sources.fundamentals.yahoo_finance import YahooFinanceFundamentalsAdapter
from src.database.models.company import Company
from src.ingestion.fundamentals import ingest_fundamentals


def cmd_fundamentals_sync(
    session: Session,
    settings: Settings,
    offset: int = 0,
    limit: int | None = None,
    tickers: list[str] | None = None,
) -> int:
    run = _start_pipeline_run(session, "fundamentals_sync")
    if tickers:
        companies = [c for c in (session.scalar(select(Company).where(Company.ticker == t)) for t in tickers) if c]
    else:
        all_companies = list(session.scalars(select(Company).order_by(Company.ticker)))
        companies = all_companies[offset : offset + limit] if limit is not None else all_companies[offset:]
    if not companies:
        _finish_pipeline_run(session, run, 0, 0, "no companies in database")
        print("FAILED: no companies in database.")
        return 1

    provider = YahooFinanceFundamentalsAdapter()
    total_written = 0
    total_skipped = 0

    for company in companies:
        outcome = ingest_fundamentals(session, provider, company.ticker, run.run_uuid)
        session.commit()
        if outcome.skipped_reason:
            total_skipped += 1
            print(f"{company.ticker}: SKIPPED ({outcome.skipped_reason})")
            continue
        print(
            f"{company.ticker}: statements={outcome.statements_written} "
            f"items={outcome.items_written}"
        )
        total_written += outcome.statements_written

    _finish_pipeline_run(session, run, total_written, total_skipped, None)
    print(
        f"\nFundamentals-sync summary: {len(companies)} companies, {total_written} statements written, "
        f"{total_skipped} skipped"
    )
    return 0
